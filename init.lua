...
copyright: Copyright © 2020 HeNan DtCloud Network Technology Co.,Lt d.
--]]
local set = vim.o
set.number = true
...
local function on_new_config(new_config, new_root_dir)
	local function get_typescript_server_path(root_dir)
		...
		local function check_dir(path)
			find_ts = lspconfig_util.path.join(path, "node_modules", "typescript", "lib")
			if lspconfig_util.path.exists(find_ts) then
				return path
			end
		end
		if lspconfig_util.search_ancestors(new_root_dir, check_dir) then
			return find_ts
		else
			return global_ts
		end
	...
	-- or "/usr/local/lib/node_modules/typescript/lib"
	-- end
